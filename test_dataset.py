...
def load_dataset(config, settings_file_path, data, path_model = None, drdv = False):
    ...
    if drdv == False:
        ...
        test_dxdt_pred = ANN_tf.drdv(test_x)
...
def plot_prediction_error_HNNvsDNN(path_figure, x, y_pred, y_real, x2, y_pred2, y_real2, x3, y_pred3, y_real3):
    ...
    for sbu in range(subplot1):
        ...
        for sbu2 in range(subplot2):
            ...
            ax[sbu2].scatter(y_real3[:, var], y_pred3[:, var], label = 'HNN asteroid in loss', color = color[5], marker = 's', s = 10, zorder = 3)
            ax[sbu2].scatter(y_real2[:, var], y_pred2[:, var], label = 'DNN', color = color[9], marker = 'x', s = 10, zorder =4)
            ...
            ax[sbu2].set_xticklabels(np.round(trunc(ax[sbu2].get_xticks(), decs = 5), decimals = dec+1), rotation = 20, fontsize = 16)
            # ax[sbu2].set_xticklabels(trunc(ax[sbu2].get_xticks(), decs = 5), rotation = 20, fontsize = 14)
            ticks = -np.log10(ax[sbu2].get_yticks())
            dec = int(np.round(np.nanmax(ticks[ticks!= np.inf]), 0) )
            ax[sbu2].set_yticklabels(np.round(trunc(ax[sbu2].get_yticks(), decs = 5), decimals = dec+1), rotation = 0, fontsize = 16)
