...
#         dt = WisdomHolman.initial_step_size(x0, self._particles.masses, self.particles.N, 1, factor)
        ...
        while t_current < to_time:
            helio, accel, jacobi = WisdomHolman.wh_advance_step(helio, t_current, self.h, self.particles.masses, self.particles.N, accel, self.CONST_G)
            t_current += self.h

            # save training data
#             self.coord.append(np.append(helio, jacobi))

            # Save data in jacobi
            q = jacobi[0:3*self.particles.N].reshape(self.particles.N, 3)
            v = jacobi[3*self.particles.N:].reshape(self.particles.N,3)
            p = np.multiply(jacobi[3*self.particles.N:].reshape(self.particles.N,3).T, self.particles.masses).T

            # Save data in heliocentric
            # q = helio[0:3*self.particles.N].reshape(self.particles.N, 3)
            # p = np.multiply(helio[3*self.particles.N:].reshape(self.particles.N,3).T, self.particles.masses).T

            self.coord.append(np.append(q, p, axis=1))
            dq = p
            a = accel.reshape(self.particles.N, 3)
            self.dcoord.append(np.append(p, a, axis=1))
        self._t = to_time
        self.particles.positions = helio[0:3*self.particles.N]
        self.particles.velocities = helio[3*self.particles.N:]
        self.store_state()
