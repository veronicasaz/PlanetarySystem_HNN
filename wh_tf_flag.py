    ...
    def wh_advance_step(self, x, t, dt, masses, nbodies, accel, G, nih=False):
        ...
        if nih is False:
            ...
            if self.multiple_nets == 'Asteroid_JS':  # Case with Sun, Jupiter, Saturn and asteroids          
                ...
                input_nn = np.zeros((nbodies-3, 4*3)) # Each input is with 3 bodies
                
                # Mass of asteroid included
                input_nn[:, 0:8] = np.tile(jacobi_input[0:2, :].flatten(),(np.shape(input_nn)[0],1))
                input_nn[:, 8:] = jacobi_input[2:, :]
                accel = np.zeros(np.shape(accel))
                accel[3:9] = self.hnn[0].predict(input_JS)
                ...
                if self.flag == True:
                    ...
                    jacobi_vel =  jacobi[3*nbodies:].reshape((nbodies, 3))

                    if np.any(idx_bad <3) : # If jup or saturn are bad, redo
                ...
                input_nn[:, 8:] = jacobi_input[2:, :]
                accel = np.zeros(np.shape(accel))
                accel[3:9] = self.hnn[0].predict(input_JS, std = False)
                ...
                if self.flag == True:
                    ...
                    comp_e0 = np.abs((energy_0-self.__energy_init)/self.__energy_init)
                    helio = WisdomHolman.wh_kick(helio, dt / 2, masses, nbodies, accel)
                    energy = self.compute_energy(helio, masses, nbodies, G)
                    comp_e = np.abs((energy-self.__energy_init)/self.__energy_init)
                    if np.abs(comp_e - comp_e0) > 1e-5:
                        accel = WisdomHolman.compute_accel(helio, jacobi, masses, nbodies, G)
                        flag_list = np.ones(nbodies)

            elif self.multiple_nets == 'JS': # Case with Sun, Jupiter, and Saturn 
                q = jacobi[0:3*nbodies].reshape(nbodies, 3)
                input_nn = np.hstack((np.reshape(masses[1:], (-1,1)), q[1:,:])).flatten()
                accel_prev = accel # Save value of previous acceleration for flag
                ...
                input_nn = np.zeros((nbodies-3, 4*3)) # Each input is with 3 bodies
                input_nn[:, 0] = jacobi_input[2:, 0]
                input_nn[:, 0:8] = np.tile(jacobi_input[0:2, :].flatten(),(np.shape(input_nn)[0],1))
                input_nn[:, 8:] = jacobi_input[2:, :]
                accel = np.zeros(np.shape(accel))
                output_nn = self.hnn.predict(input_nn, std = False)
                accel[3:9] = np.array(np.mean(output_nn[:, 0:6], axis = 0))
                accel[9:] = np.array(output_nn[:, 6:]).flatten()
